...
set -euo pipefail
printf '%s\n' "$*" >> "$TEST_CARGO_LOG"
packages=(tidas-contracts tidas-runtime tidas-xml tidas-references tidas-assets tidas-conversion tidas-rulesets tidas-validation tidas-export tidas-import tidas-release tidas)
case "$1" in
  metadata)
    printf '%s\n' "${packages[@]}" | jq -Rn --arg target "${TEST_TARGET:-}" '{target_directory:$target,packages:[inputs|{name:.,version:"0.3.0",publish:["crates-io"]}]}'
    ;;
  package)
...
[[ "$(grep -c '^qualified ' "$fixture/custom.log")" == 12 ]] || fail 'incomplete package qualification'
